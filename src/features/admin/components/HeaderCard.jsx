...
const HeaderCard = ({ title, subtitle, showBack = false, onBack }) => {
  return (
    <section
    ...
    >
      ...
      <div className={showBack ? "pt-14 relative z-10" : "relative z-10"}>
        <h1 className="text-4xl font-extrabold tracking-wide drop-shadow-lg transition-all duration-500 ease-out group-hover:text-white group-hover:[text-shadow:0_0_20px_rgba(96,165,250,0.8),0_0_40px_rgba(59,130,246,0.6),0_0_60px_rgba(37,99,235,0.4)]">
          {title}
        </h1>

        {subtitle && (
          <p className="text-lg mt-3 text-zinc-300 font-medium transition-all duration-500 ease-out group-hover:text-white group-hover:[text-shadow:0_0_15px_rgba(147,197,253,0.7),0_0_30px_rgba(96,165,250,0.5)]">
            {subtitle}
          </p>
        )}
      </div>
